...
`ufo-playground`: `ufodb_v0`（Union-Find DB）をブラウザだけで試せるWebアプリ。UFO Studio（Tauri製デスクトップアプリ）と同じUI・同じGUI操作を、インストールなしで提供する。
...
- `src/` — React + TypeScript（Vite）。UIコンポーネントは`ufo-design-system`から使い、このリポジトリではWASMとの接続と画面の組み立てを行う
...
- **`ufo-design-system`の参照方法**: ローカル開発では`link:`/`file:`。CIでビルドする段階でgit依存（publicなので認証不要）かnpm公開に切り替える
...
- `ufo-design-system`: 共通のReactコンポーネントとデザイントークン。UIの変更は基本的にそちらで行う
- `toy_ufdb_gui_app`（UFO Studio）: 同じUIを使うTauri製デスクトップアプリ
